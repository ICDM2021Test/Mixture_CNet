...
class CLT:
    ...
    def update_exact(self, dataset_, weights=np.array([]), structure_update_flag = False):
        ...
        #print (dataset_.shape[0])
        #print (np.sum(weights))
        if weights.shape[0]==dataset_.shape[0] and np.sum(weights > 0):
        #if weights.shape[0]==dataset_.shape[0]:
            #self.xycounts += Util.compute_weighted_xycounts(dataset_, weights)
            #self.xcounts += Util.compute_weighted_xcounts(dataset_, weights)
    ...
    def get_log_cond_cpt(self):
        ...
        #self.log_cond_cpt = np.log(Util.compute_conditional_CPT(self.xyprob,self.xprob,self.topo_order, self.parents))
    ...
    FOR CUTSET_deep
    """
#    # set the evidence in log
#    def instantiation_log(self, evid_list):
#        #print ('in instantiation')
#        #print (evid_list)
#        
#        ret_log_cond_cpt = np.copy(self.log_cond_cpt)
#        #print ("before:")
#        #print (cond_cpt)
#        for i in xrange (len(evid_list)):
#            variable_id = evid_list[i][0]
...
#            # set varible value = 0
#            if value == 0:                    
#                ret_log_cond_cpt[index_c, 1,:] = utilM.LOG_ZERO
#                ret_log_cond_cpt[index_p, :,1] = utilM.LOG_ZERO
#            
#            # set varible value = 1                   
#            elif value == 1:                    
#                ret_log_cond_cpt[index_c, 0,:] = utilM.LOG_ZERO
#                ret_log_cond_cpt[index_p, :,0] = utilM.LOG_ZERO
#
#            else:
#                print ('error in value: ', value)
#                exit()
...
#        return ret_log_cond_cpt
    ...
    def get_kl_divergence(self, edges, log_edge_potential):
        ...
        for i,e in enumerate(edges):
            #print ('edges: ', e)
            #print ('log_edge_potential: ', log_edge_potential[i])

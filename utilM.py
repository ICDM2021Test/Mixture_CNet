...
def get_edge_prob(topo_order, parents, cond_cpt, edges):
    ...
    #cpt_income = np.ones((topo_order.shape[0],2))
    for i in xrange (edges.shape[0]):
        x = edges[i,0]
        y = edges[i,1]
        #xyprob[i,:,:] = ve_tree_bin2(topo_order, parents, cond_cpt, x, y, cpt_income)
